...
const AllUsers = () => {
  ...
  } = useQuery([BrandAPI], () => request({ url: BrandAPI, params: { status: 1 } }, router), {
    enabled: false,
  ...
  } = useQuery([store], () => request({ url: store, params: { status: 1 } }, router), {
    enabled: false,
    refetchOnWindowFocus: false,
    select: (res) =>
      res?.data?.data?.map((elem) => {
  ...
  return (
    <Col sm="12">
      <Formik initialValues={{ category_ids: [], brand_ids: [], store_ids: [], product_type: "" }}>
          ...
          <Form>
            <AllProductTable
              ...
              importExport={{
                ...
                paramsProps: {
                  category_ids:
                    values["category_ids"].length > 0 ? values.category_ids.join(",") : null,
