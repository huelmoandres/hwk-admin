...
const AddShippingRules = ({ params }) => {
  ...
  const { data, isLoading, refetch } = useQuery(
    ...
    () => request({ url: `${shipping}/${updateId}` }, router),
    {
      enabled: false,
  ...
  const { mutate: createMutate, isLoading: createMutateIsloading } = useMutation(
    (data) => request({ url: shippingRule, data, method: "post" }, router),
    {
      onSuccess: (resData) => {
        SuccessHandle(
          resData,
  ...
  const { mutate: updateMutate, isLoading: updateLoading } = useMutation(
    (data) => request({ url: shippingRule + "/" + active, method: "put", data }, router),
    {
      onSuccess: (resData) => {
