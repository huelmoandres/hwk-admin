        ...
        differentFilter={
          <div className="show-box mb-4 d-flex overflow-custom">
            <ul className="order-tab-content">
              <li onClick={() => setStatus(null)} className={`${!statusValue ? "active" : ""}`}>
                <Link href={`/qna`}> All </Link>
              </li>
              <li
                onClick={() => setStatus("pending")}
                className={`${statusValue === "pending" ? "active" : ""}`}
              >
                <Link href={{ pathname: `/qna`, query: { status: "pending" } }}> pending </Link>
              </li>
              <li
                onClick={() => setStatus("answered")}
                className={`${statusValue === "answered" ? "active" : ""}`}
              >
                <Link href={{ pathname: `/qna`, query: { status: "answered" } }}> Answered </Link>
              </li>
            </ul>
          </div>
        }
