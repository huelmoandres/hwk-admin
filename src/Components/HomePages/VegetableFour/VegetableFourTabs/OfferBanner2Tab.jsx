...
const OfferBanner2Tab = ({ values, setFieldValue, productData, categoryData, setSearch }) => {
  ...
  const removeBanners = (index) => {
    if (values["content"]["offer_banner_2"]["banners"].length > 1) {
      let filterValue = values["content"]["offer_banner_2"]["banners"].filter(
        (item, i) => i !== index
      );
      setFieldValue("[content][banner][banners]", filterValue);
      filterValue?.forEach((elem, i) => {
        elem?.image_url &&
          setFieldValue(`offerBanner2Image${i}`, { original_url: elem?.image_url });
        elem?.redirect_link?.link_type &&
          setFieldValue(`offerBanner2RedirectLinkType${i}`, elem?.redirect_link?.link_type);
        elem?.redirect_link?.link &&
          setFieldValue(`offerBanner2RedirectLink${i}`, elem?.redirect_link?.link);
      });
    }
  };
  return (
    <>
      {
        <Btn
          ...
          onClick={() =>
            setFieldValue("[content][offer_banner_2][banners]", [
              ...values["content"]?.["offer_banner_2"]["banners"],
              { title: "", description: "" },
            ])
          }
          title="AddBanner"
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <FileUploadField
                      ...
                      name={`offerBanner2Image${index}`}
                      title="Image"
                      id={`offerBanner2Image${index}`}
                      type="file"
                      values={values}
                      setFieldValue={setFieldValue}
                      showImage={values[`offerBanner2Image${index}`]}
                    ...
                    <CommonRedirect
                      ...
                      nameList={{
                        selectNameKey: `offerBanner2RedirectLinkType${index}`,
                        multipleNameKey: `offerBanner2RedirectLink${index}`,
                      }}
                      setSearch={setSearch}
                    />
                    <CheckBoxField
                      name={`[content][offer_banner_2][banners][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
