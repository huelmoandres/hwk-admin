...
const AboutUsTab = ({ values, setFieldValue }) => {
  ...
  const removeBanners = (index) => {
    if (values["content"]["about_us"]["services"]["banners"].length > 1) {
      let filterValue = values["content"]["about_us"]["services"]["banners"].filter(
        (item, i) => i !== index
      );
      setFieldValue("[content][about_us][services][banners]", filterValue);
      filterValue?.forEach((elem, i) => {
        elem?.image_url &&
          setFieldValue(`serviceBannerImage${i}`, { original_url: elem?.image_url });
      });
    }
  };
  return (
    <>
      <SimpleInputField
        nameList={[
          ...
          {
            name: `[content][about_us][description]`,
      ...
      />
      {
        <Btn
          ...
          onClick={() =>
            setFieldValue("[content][about_us][services][banners]", [
              ...values["content"]?.["about_us"]["services"]["banners"],
              { title: "", description: "" },
            ])
          }
          title="AddBanner"
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <SimpleInputField
                      nameList={[
                        {
                          name: `[content][about_us][services][banners][${index}][title]`,
                    ...
                    <FileUploadField
                      paramsProps={{ mime_type: mediaConfig.image.join(",") }}
                      name={`serviceBannerImage${index}`}
                      title="Image"
                      id={`serviceBannerImage${index}`}
                      type="file"
                      values={values}
                      setFieldValue={setFieldValue}
                      showImage={values[`serviceBannerImage${index}`]}
                      helpertext={getHelperText("375x586px")}
                    />
                    <CheckBoxField
                      name={`[content][about_us][services][banners][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
