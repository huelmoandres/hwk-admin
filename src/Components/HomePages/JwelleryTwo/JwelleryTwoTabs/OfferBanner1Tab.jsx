...
const OfferBanner1Tab = ({ values, setFieldValue, productData, categoryData, setSearch }) => {
  ...
  const removeBanners = (index) => {
    if (values["content"]["offer_banner_1"]["banners"].length > 1) {
      let filterValue = values["content"]["offer_banner_1"]["banners"].filter(
        (item, i) => i !== index
      );
      setFieldValue("[content][offer_banner_1][banners]", filterValue);
      filterValue?.forEach((elem, i) => {
        elem?.image_url &&
          setFieldValue(`offerBanner1Image${i}`, { original_url: elem?.image_url });
        elem?.redirect_link?.link_type &&
          setFieldValue(`offerBanner1RedirectLinkType${i}`, elem?.redirect_link?.link_type);
        elem?.redirect_link?.link &&
          setFieldValue(`offerBanner1RedirectLink${i}`, elem?.redirect_link?.link);
      });
    }
  };
  return (
    <>
      {
        <Btn
          ...
          onClick={() =>
            setFieldValue("[content][offer_banner_1][banners]", [
              ...values["content"]?.["offer_banner_1"]["banners"],
              { title: "", description: "" },
            ])
          }
          title="AddBanner"
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <FileUploadField
                      ...
                      name={`offerBanner1Image${index}`}
                      title="Image"
                      id={`offerBanner1Image${index}`}
                      type="file"
                      values={values}
                      setFieldValue={setFieldValue}
                      showImage={values[`offerBanner1Image${index}`]}
                    ...
                    <CommonRedirect
                      ...
                      nameList={{
                        selectNameKey: `offerBanner1RedirectLinkType${index}`,
                        multipleNameKey: `offerBanner1RedirectLink${index}`,
                      }}
                      setSearch={setSearch}
                    />
                    <CheckBoxField
                      name={`[content][offer_banner_1][banners][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
