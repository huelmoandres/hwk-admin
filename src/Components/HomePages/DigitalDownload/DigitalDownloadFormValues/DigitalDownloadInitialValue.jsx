const DigitalDownloadInitialValue = (data) => {
  ...
  return {
    ...
    homeBannerBackgroundImage: data?.content?.home_banner?.background_image
      ? { original_url: data?.content?.home_banner?.background_image }
      : "",
    homeBannerSubImage1: data?.content?.home_banner?.sub_image_1
      ? { original_url: data?.content?.home_banner?.sub_image_1 }
      : "",
    homeBannerSubImage2: data?.content?.home_banner?.sub_image_1
      ? { original_url: data?.content?.home_banner?.sub_image_1 }
      : "",

    productListMainImage: data?.content?.products_list_2?.image_url
      ? { original_url: data?.content?.products_list_2?.image_url }
      : "",
    ...obj,

    // Redirect Link
    productListLeftContentLinkType:
      data?.content?.products_list_2?.left_panel?.redirect_link?.link_type || "",
    productListLeftContentLink:
      data?.content?.products_list_2?.left_panel?.redirect_link?.link || "",
