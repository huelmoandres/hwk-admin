...
const ParallaxBannerTab = ({ values, setFieldValue, productData, categoryData, setSearch }) => {
  ...
  const removeBanners = (index) => {
    if (values["content"]["parallax_banner"]["banners"].length > 1) {
      let filterValue = values["content"]["parallax_banner"]["banners"].filter(
        (item, i) => i !== index
      );
    ...
    <>
      {
        <Btn
          ...
          onClick={() =>
            setFieldValue("[content][parallax_banner][banners]", [
              ...values["content"]?.["parallax_banner"]["banners"],
              { title: "", description: "" },
            ])
          }
          title="AddBanner"
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <SimpleInputField
                      nameList={[
                        {
                          name: `[content][parallax_banner][banners][${index}][main_title]`,
                          placeholder: t("EnterMainTitle"),
                          title: "MainTitle",
                        },
                        {
                          name: `[content][parallax_banner][banners][${index}][title]`,
                          placeholder: t("EnterTitle"),
                          title: "Title",
                        },
                        {
                          name: `[content][parallax_banner][banners][${index}][sub_title]`,
                    ...
                    <FileUploadField
                      ...
                      name={`parallaxBannerImage${index}`}
                      title="Image"
                      id={`parallaxBannerImage${index}`}
                      type="file"
                      values={values}
                      setFieldValue={setFieldValue}
                      showImage={values[`parallaxBannerImage${index}`]}
                      helpertext={getHelperText("376x231px")}
                    />
                    <CheckBoxField
                      name={`[content][parallax_banner][banners][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
