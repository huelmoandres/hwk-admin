...
const SocialMediaTab = ({ values, setFieldValue, productData, categoryData, setSearch }) => {
  ...
  const removeBanners = (index) => {
    if (values["content"]["social_media"]["banners"].length > 1) {
      let filterValue = values["content"]["social_media"]["banners"].filter(
        (item, i) => i !== index
      );
      setFieldValue("[content][home_banner][banners]", filterValue);
      filterValue?.forEach((elem, i) => {
        elem?.image_url &&
          setFieldValue(`socialMediaBannerImage${i}`, { original_url: elem?.image_url });
        elem?.redirect_link?.link_type &&
          setFieldValue(`socialMediaBannerRedirectLinkType${i}`, elem?.redirect_link?.link_type);
        elem?.redirect_link?.link &&
          setFieldValue(`socialMediaBannerRedirectLink${i}`, elem?.redirect_link?.link);
      });
    }
  };
  ...
  return (
    <>
      <SimpleInputField
        nameList={[
          { name: `[content][social_media][title]`, placeholder: t("EnterTitle"), title: "Title" },
        ]}
      />
      {
        <Btn
          ...
          onClick={() =>
            setFieldValue("[content][social_media][banners]", [
              ...values["content"]?.["social_media"]["banners"],
              { title: "", description: "" },
            ])
          }
          title="AddBanner"
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <FileUploadField
                      ...
                      name={`socialMediaBannerImage${index}`}
                      title="Image"
                      id={`socialMediaBannerImage${index}`}
                      type="file"
                      values={values}
                      setFieldValue={setFieldValue}
                      showImage={values[`socialMediaBannerImage${index}`]}
                    ...
                    <CommonRedirect
                      ...
                      nameList={{
                        selectNameKey: `socialMediaBannerRedirectLinkType${index}`,
                        multipleNameKey: `socialMediaBannerRedirectLink${index}`,
                      }}
                      setSearch={setSearch}
                    />
                    <CheckBoxField
                      name={`[content][social_media][banners][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
