...
const OfferBannerTab = ({ values, setFieldValue, productData, categoryData, setSearch }) => {
  ...
  const removeBanners = (index) => {
    if (values["content"]["offer_banner"]["banners"].length > 1) {
      let filterValue = values["content"]["offer_banner"]["banners"].filter(
        (item, i) => i !== index
      );
      ...
      filterValue?.forEach((elem, i) => {
        ...
        elem?.redirect_link?.link_type &&
          setFieldValue(`offerBannerRedirectLinkType${i}`, elem?.redirect_link?.link_type);
        elem?.redirect_link?.link &&
          setFieldValue(`offerBannerRedirectLink${i}`, elem?.redirect_link?.link);
      });
    }
  };
  return (
    <>
      {
        <Btn
          ...
          onClick={() =>
            setFieldValue("[content][offer_banner][banners]", [
              ...values["content"]?.["offer_banner"]["banners"],
              { title: "", description: "" },
            ])
          }
          title="AddBanner"
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <FileUploadField
                      ...
                      name={`offerBannerImage${index}`}
                      title="Image"
                      id={`offerBannerImage${index}`}
                      type="file"
                      values={values}
                      setFieldValue={setFieldValue}
                      showImage={values[`offerBannerImage${index}`]}
                    ...
                    <CommonRedirect
                      ...
                      nameList={{
                        selectNameKey: `offerBannerRedirectLinkType${index}`,
                        multipleNameKey: `offerBannerRedirectLink${index}`,
                      }}
                      setSearch={setSearch}
                    />
                    <CheckBoxField
                      name={`[content][offer_banner][banners][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
