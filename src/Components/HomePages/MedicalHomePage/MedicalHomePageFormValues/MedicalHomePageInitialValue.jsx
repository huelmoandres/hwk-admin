const MedicalHomePageInitialValue = (data) => {
  ...
  data?.content?.offer_banner?.banners?.length > 0 &&
    data?.content?.offer_banner?.banners?.forEach((elem, index) => {
      ...
      elem?.redirect_link
        ? (obj[`offerRedirectLinkType${index}`] = elem?.redirect_link?.link_type)
        : "";
  ...
  return {
    ...
    // Image
    offerBanner1Image: data?.content?.column_banner_product?.offer_banner_1?.image_url
      ? { original_url: data?.content?.column_product_banner?.offer_banner_1?.image_url }
      : "",
    offerBanner2Image: data?.content?.column_banner_product?.offer_banner_2?.image_url
