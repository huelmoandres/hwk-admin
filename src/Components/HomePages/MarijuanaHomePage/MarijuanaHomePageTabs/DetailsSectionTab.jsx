...
const DetailsSectionTab = ({ values, setFieldValue, productData, categoryData, setSearch }) => {
  ...
  const removeBanners = (index) => {
    if (values["content"]["details_section"]["banners"].length > 1) {
      let filterValue = values["content"]["details_section"]["banners"].filter(
        (item, i) => i !== index
      );
      setFieldValue("[content][details_section][banners]", filterValue);
      filterValue?.forEach((elem, i) => {
        elem?.image_url &&
          setFieldValue(`detailsSectionImage${i}`, { original_url: elem?.image_url });
        elem?.redirect_link?.link_type &&
          setFieldValue(`detailsSectionRedirectLinkType${i}`, elem?.redirect_link?.link_type);
        elem?.redirect_link?.link &&
          setFieldValue(`detailsSectionRedirectLink${i}`, elem?.redirect_link?.link);
      });
    }
  };
    ...
    <>
      {
        <Btn
          ...
          onClick={() =>
            setFieldValue("[content][details_section][banners]", [
              ...values["content"]?.["details_section"]["banners"],
              { title: "", description: "" },
            ])
          }
          title="AddBanner"
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <SimpleInputField
                      nameList={[
                        {
                          name: `[content][details_section][banners][${index}][title]`,
                          placeholder: t("EnterTitle"),
                          title: "Title",
                        },
                        {
                          name: `[content][details_section][banners][${index}][description]`,
                    ...
                    <FileUploadField
                      ...
                      name={`detailsSectionImage${index}`}
                      title="Image"
                      id={`detailsSectionImage${index}`}
                      type="file"
                      values={values}
                      setFieldValue={setFieldValue}
                      showImage={values[`detailsSectionImage${index}`]}
                      helpertext={getHelperText("376x231px")}
                    />
                    <CheckBoxField
                      name={`[content][details_section][banners][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
