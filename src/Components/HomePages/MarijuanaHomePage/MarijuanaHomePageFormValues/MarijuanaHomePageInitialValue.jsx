const MarijuanaHomePageInitialValue = (data) => {
  ...
  return {
    ...
    slug: data?.slug,
    // Images
    offerBanner1Image: data?.content?.offer_banner?.banner?.banner_1?.image_url
      ? { original_url: data?.content?.offer_banner?.banner?.banner_1?.image_url }
      : "",

    offerBanner2Image: data?.content?.offer_banner?.banner?.banner_2?.image_url
      ? { original_url: data?.content?.offer_banner?.banner?.banner_2?.image_url }
      : "",

    categoryProductImage: data?.content?.category_product?.image_url
      ? { original_url: data?.content?.category_product?.image_url }
      : "",
    ...obj,

    // Redirect Link
    offerBanner1LinkType:
      data?.content?.offer_banner?.banner?.banner_1?.redirect_link?.link_type || "",
    offerBanner1Link: data?.content?.offer_banner?.banner?.banner_1?.redirect_link?.link || "",

    offerBanner2LinkType:
      data?.content?.offer_banner?.banner?.banner_2?.redirect_link?.link_type || "",
