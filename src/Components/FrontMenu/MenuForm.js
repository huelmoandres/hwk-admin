...
const MenuForm = ({ updateId, loading }) => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery(["menu/" + updateId], () => request({ url: `menu/${updateId}` }), {
    enabled: false,
  });
  const {
    data,
  ...
  } = useQuery([PagesAPI], () => request({ url: PagesAPI }, router), {
    enabled: false,
    refetchOnWindowFocus: false,
    select: (res) =>
      res?.data?.data?.map((elem) => {
        return { id: elem?.slug, name: elem?.title, slug: elem.slug };
      }),
  });
  useEffect(() => {
    updateId && refetch();
      ...
      {({ setFieldValue, values, errors }) => (
        <Form className="theme-form theme-form-2 mega-form">
          <Row>
            ...
            {!values["parent_id"] && values["mega_menu"] && (
              <FrontMenuRadio values={values} setFieldValue={setFieldValue} />
            )}
            {!values["parent_id"] &&
              values["mega_menu"] &&
              (values["mega_menu_type"] === "side_banner" ||
                values["mega_menu_type"] === "bottom_banner" ||
                values["mega_menu_type"] === "product_box" ||
                values["mega_menu_type"] === "blog_box") && (
                <ChildrenCategory values={values} setFieldValue={setFieldValue} />
              )}

            {updatedData?.find((data) => data.id == values["parent_id"])?.mega_menu_type ==
              "link_with_image" &&
              values["parent_id"] && (
                <FileUploadField
                  name="item_image_id"
                  title="link_image"
                  id="item_image_id"
                  showImage={values["item_image_id"]}
                  type="file"
                  values={values}
                  setFieldValue={setFieldValue}
