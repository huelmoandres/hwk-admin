...
const VariationTop = ({ data, index, values, setFieldValue, errors }) => {
  ...
  const setAttributesValues = (name, value) => {
    ...
    setFieldValue(
      "combination",
      values["combination"].map((item, i) => (i == index ? { ...item, values: value } : item))
    );
  };
  const handleRemove = () => {
  ...
  return (
    <div className="variant-row">
        ...
        nameList={[
          {
            ...
            inputprops: {
              ...
              options: data?.filter((item) =>
                values["combination"].find((elem) => elem?.name?.name == item.name) ? false : true
              ),
      ...
      <MultiSelectField
        ...
        name="values"
        title="Value"
        data={values?.["combination"]?.[index]?.["name"]?.["attribute_values"]?.map((elem) => ({
          name: elem.value,
          id: elem.id,
        }))}
      />
