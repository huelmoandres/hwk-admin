...
const BlogForm = ({ updateId, buttonName }) => {
  ...
  const { data } = useQuery(
    [Category],
    () => request({ url: Category, params: { type: "post" } }, router),
    { refetchOnWindowFocus: false, select: (data) => data.data.data }
  );
  const { data: tagData } = useQuery(
    [tag],
    () => request({ url: tag, params: { type: "post" } }, router),
    { refetchOnWindowFocus: false, select: (data) => data.data.data }
  );
  const {
    data: oldData,
        ...
        {({ values, setFieldValue, errors, touched }) => (
          <>
            <Form id="blog" className="theme-form theme-form-2 mega-form">
              <SimpleInputField
                nameList={[{ name: "title", placeholder: t("EnterBlogTitle"), require: "true" }]}
              />
              <SimpleInputField
                nameList={[{ name: "description", placeholder: t("EnterBlogDescription") }]}
              />
              <DescriptionInput
                values={values}
              ...
              />
              <FileUploadField
                name="blog_thumbnail_id"
                title="Thumbnail"
                require="true"
                id="blog_thumbnail_id"
                updateId={updateId}
                type="file"
                values={values}
                setFieldValue={setFieldValue}
              ...
              />
              <FileUploadField
                name="blog_meta_image_id"
                title="BlogMetaImage"
                id="blog_meta_image_id"
                updateId={updateId}
                type="file"
                values={values}
                setFieldValue={setFieldValue}
