...
const BrandForm = ({ updateId, buttonName }) => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery([updateId], () => request({ url: BrandAPI + "/" + updateId }, router), {
        ...
        {({ values, setFieldValue, errors, touched }) => (
          <>
            <Form id="blog" className="theme-form theme-form-2 mega-form">
              <SimpleInputField
                nameList={[{ name: "name", placeholder: t("EnterName"), require: "true" }]}
              />
              <FileUploadField
              ...
              <FileUploadField
                paramsProps={{ mime_type: mediaConfig.image.join(",") }}
                name="brand_meta_image_id"
                id="brand_meta_image_id"
                title="meta_image"
                updateId={updateId}
                type="file"
                values={values}
