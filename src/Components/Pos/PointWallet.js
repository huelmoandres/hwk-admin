  ...
  return (
    <>
      {values["isPoint"]?.balance > 0 && data?.data?.total?.convert_point_amount && (
        <>
          <li>
            <h4 className={`${values["points_amount"] ? "fw-bold txt-primary" : "text-muted"}`}>
              {t("Points")}
            </h4>
            <h4
      ...
      {values["isWallet"]?.balance > 0 && data?.data?.total?.convert_wallet_balance && (
        <>
          <li>
            <h4 className={`${values["wallet_balance"] ? "fw-bold txt-primary" : "text-muted"}`}>
              {t("WalletBalance")}
            </h4>
            <h4
              className={`${values["wallet_balance"] ? "price fw-bold txt-primary" : "price text-muted"}`}
            >
              {convertCurrency(data?.data?.total?.convert_wallet_balance || 0)}
            </h4>
          </li>
          <li className="border-cls">
