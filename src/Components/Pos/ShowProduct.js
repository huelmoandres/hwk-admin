  ...
  useEffect(() => {
    ...
    if (data?.data?.items) {
      ...
      if (index == -1) {
        ...
        productCopy[index].sub_total =
          productCopy[index].quantity * productCopy[index].product.sale_price;
      ...
      setFieldValue(
        "products",
        index !== -1 ? cart : [...values["products"], ...data?.data?.items]
      );
    }
  }, [data, isLoading]);
    ...
    <Col>
      <div className="product-box">
        <div className="product-image">
          <Image
            src={productData?.product_thumbnail?.original_url ?? placeHolderImage}
            alt="product Images"
            className="img-fluid"
            width={100}
            height={100}
        ...
        <div className="product-detail">
          <h6
            ...
            onClick={() =>
              window.open(`${settingObj?.general?.site_url}/product/${productData.slug}`)
            }
          >
            {productData.name}
          </h6>{" "}
            ...
            <div className="addtocart_btn">
              {productData?.product_type != "external" ? (
                productData?.stock_status == "in_stock" ? (
                  <Btn
                    onClick={() => addProduct(productData, 1)}
                    className={"add-button addcart-button btn buy-button"}
                    id={productData.type === "classified" ? "" : "addcartbtn" + productData?.id}
                  >
                    {t("Add")} <RiAddLine />
                  </Btn>
                ) : (
                  <Btn
                    disabled={true}
                    className="add-button addcart-button btn buy-button disabled"
                  >
                    {" "}
                    {t("Outofstock")}{" "}
                  </Btn>
                ...
                <Btn
                  onClick={() => window.open(productData?.external_url, "_blank")}
                  className="add-button addcart-button btn buy-button"
                >
                  {productData?.external_button_text
                    ? productData.external_button_text
                    : t("Buynow")}{" "}
                </Btn>
              )}
      ...
      <ShowModal
        open={productData.id == modal}
        setModal={setModal}
        modalAttr={{ className: "view-modal modal-lg theme-modal" }}
      >
