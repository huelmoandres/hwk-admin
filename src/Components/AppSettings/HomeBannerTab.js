    ...
    if (values["values"]["home_banner"]["banners"].length > 1) {
      ...
      filterValue?.forEach((elem, i) => {
        ...
        elem?.redirect_link?.link_type &&
          setFieldValue(`home_bannerRedirectLinkType${i}`, elem?.redirect_link?.link_type);
        elem?.redirect_link?.link &&
          setFieldValue(`home_bannerRedirectLink${i}`, elem?.redirect_link?.link);
      });
    }
  };
  return (
    <>
      {
        <Btn
          ...
          onClick={() =>
            setFieldValue("[values][home_banner][banners]", [
              ...values["values"]["home_banner"]["banners"],
              { title: "", description: "" },
            ])
          }
          title="AddBanner"
              ...
              <div className="shipping-accordion-custom">
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <FileUploadField
                      name={`home_bannerImage${index}`}
                      title="Image"
                      id={`home_bannerImage${index}`}
                      type="file"
                      values={values}
                      setFieldValue={setFieldValue}
                      showImage={values[`home_bannerImage${index}`]}
                    ...
                    <CommonRedirect
                      ...
                      nameList={{
                        selectNameKey: `home_bannerRedirectLinkType${index}`,
                        multipleNameKey: `home_bannerRedirectLink${index}`,
                      }}
                      setSearch={setSearch}
                    />
                    <CheckBoxField
                      name={`[values][home_banner][banners][${index}][status]`}
                      title="Status"
                    />
                  </div>
                )}
