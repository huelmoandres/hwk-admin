...
const ShowTable = ({
  ...
  return (
    <Table
      id="table_id"
      className={`role-table ${headerData?.noCustomClass ? "" : "refund-table"} all-package theme-table datatable-wrapper`}
    >
            ...
            {headerData?.checkBox && (
              <th className="sm-width">
                <Input
                  ...
                  type={"checkbox"}
                  checked={
                    headerData?.data?.length > 0 && isCheck?.length == headerData?.data?.length
                  }
