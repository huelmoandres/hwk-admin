...
const InvoiceSummary = ({ data }) => {
  ...
  return (
    <Card>
        ...
        <div className="title-header">
          ...
          {data?.invoice_url && (
            <div className="d-flex gap-2">
              <Btn
                className="btn-light-bg btn-sm ms-auto"
                onClick={() => setOpenReceiptModal(true)}
              >
                {t("Receipt")}
              </Btn>
              <button
                onClick={() => downloadInvoice(data?.order_number)}
      ...
      {openReceiptModal && (
        <ReceiptModal open={openReceiptModal} data={data} setOpen={setOpenReceiptModal} />
      )}
    </Card>
  );
};
