...
const OrderDetailsContain = ({ updateId }) => {
  ...
  const { data, isLoading, refetch } = useQuery(
    ["category/" + updateId],
    () => request({ url: `${OrderAPI}/${updateId}` }, router),
    {
  ...
  const {
    ...
    isLoading: orderStatusLoader,
  } = useQuery([OrderStatusAPI], () => request({ url: OrderStatusAPI }, router), {
    enabled: false,
    refetchOnWindowFocus: false,
    ...
    <Row className="pb-4">
      <Col xxl="9">
        ...
        <Col sm="12">
          <OrderNumberTable
            refetch={refetch}
            moduleName={`${t("OrderNumber")}: #${data?.order_number}`}
            data={data}
            orderStatusData={orderStatusData}
