  ...
  const handleAllCheckUncheck = (e, permissionArray) => {
    ...
    e.target.checked
      ? values.permissions.push(
          ...permissionArray.filter((permission) => !values.permissions.includes(permission))
        )
  ...
  return (
    <>
      <div className="mb-3 d-flex">
        ...
        {touched.permissions && !values.permissions.length && errors["permissions"] && (
          <div className="mt-0 ms-4  text-danger">
            {errors["permissions"] && t(`Permissionisrequired`)}
          </div>
        )}
      </div>
        ...
        <Row className="roles-form">
          {data?.map((permissionTitles, i) => (
            <Col xs="12" key={i}>
              <ul>
                <li>
                  {permissionTitles.name.includes("_")
                    ? permissionTitles.name.replace("_", " ")
                    : permissionTitles.name}
                  :
                </li>
                <li>
                  <FormGroup switch>
                    <Input
                      ...
                      name="permissions"
                      type="switch"
                      checked={permissionTitles.module_permissions.every((data) =>
                        values.permissions?.includes(data.permission_id)
                      )}
                      onChange={(e) =>
                    ...
                    />
                    <Label className="form-check-label m-0" htmlFor={permissionTitles.name}>
                      {t("All")}
                    </Label>
                  </FormGroup>
                </li>
                {permissionTitles.module_permissions.map((permissionDetails, i) => (
                  <li key={i}>
                    <FormGroup switch>
                      <Input
                        className="checkbox_animated"
                        id={"role" + permissionDetails.permission_id}
                        type="switch"
                        name="permissions"
                        checked={
                          values.permissions?.includes(permissionDetails.permission_id) && true
                        }
                        onChange={(e) =>
                          permissionDetails.name == "index" &&
                          values.permissions.includes(permissionDetails.permission_id)
                            ? handleCheckOnIndex(permissionTitles)
                            : handleCheckUncheck(
                                e,
                                permissionDetails.permission_id,
                                permissionTitles
                              )
                        }
                      />
                      <Label
                        ...
                        htmlFor={"role" + permissionDetails.permission_id}
                      >
