    ...
    <>
      {addAddress && (
        <>
          <SimpleInputField
            nameList={[
              { name: "address[0][title]", placeholder: t("EnterTitle"), title: "Title" },
              { name: "address[0][street]", placeholder: "Enter Street", title: "Street" },
            ]}
          />
          {!type && (
            ...
            nameList={[
              ...
              },
              {
                ...
                inputprops: {
                  ...
                  options: values?.["address"][0]?.["country_id"]
                    ? data.filter(
                        (country) =>
                          Number(country.id) === Number(values?.["address"][0]?.["country_id"])
                      )?.[0]?.["state"]
                    : [],
                  defaultOption: "Select state",
                },
